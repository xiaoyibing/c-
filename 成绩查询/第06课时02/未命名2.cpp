...
//----------------二分法查找-------------------
// 递归形式的查找学号
int SearchById(int targetId, struct Student students[], int left, int right) {
    if (left <= right) {
        ...
        if (students[mid].id == targetId) {
            ...
            return SearchById(targetId, students, mid + 1, right);  // 在右半部分递归查找
        } else {
            return SearchById(targetId, students, left, mid - 1);  // 在左半部分递归查找
        }
    }

    return -1;  // 没有找到目标ID，返回-1表示未找到
}

//查找平均成绩
int SearchByAverage(float targetAverage, struct Student students[], int left , int right) {
    if (left <= right) {
        int mid = left + (right - left) / 2;
        ...
        if (students[mid].average == targetAverage) {
            ...
            return SearchByAverage(targetAverage, students, mid + 1, right);  
        } else {
            return SearchByAverage(targetAverage, students, left, mid - 1);  
        }
    }
...
//查找姓名
int SearchByName(const char *targetName, struct Student students[], int left, int right) {
    if (left <= right) {
        int mid = left + (right - left) / 2;
        ...
        if (cmpResult == 0) {
            ...
            return SearchByName(targetName, students, mid + 1, right);  
        } else {
            return SearchByName(targetName, students, left, mid - 1);  
        }
    }
...
//开始排序
  ...
  //开始查找
        ...
        if (choice == 1) {
            ...
            targetIndex = SearchByAverage(targetAverage, students,0, count-1);//调用函数查找
        } else if (choice == 2) {
            int targetId;
            printf("在这里输入学号: ");
            scanf("%d", &targetId);
            targetIndex = SearchById(targetId, students, 0, count - 1);
        }else if (choice ==3){
            char targetName[50];
            printf("在这里输入姓名: ");
            scanf("%49s", &targetName);
            targetIndex = SearchByName(targetName, students,0, count-1);
		}
//输出（以及所有平均成绩）
